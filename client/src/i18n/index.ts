import i18n from "i18next";
import { initReactI18next } from "react-i18next";

// Импортируем переводы напрямую
const ruTranslation = {
  common: {
    dashboard: "Панель управления",
    tests: "Тесты",
    candidates: "Кандидаты",
    settings: "Настройки",
    admin: "Администратор",
    hr_manager: "HR менеджер",
    loading: "Загрузка...",
    filter: "Фильтр",
    search: "Найти...",
    no_data: "Нет данных",
    no_filter_results: "Нет результатов по вашему фильтру",
    actions: "Действия",
    edit: "Редактировать",
    delete: "Удалить",
    save: "Сохранить",
    cancel: "Отмена",
    success: "Успех",
    error: "Ошибка",
    app_name: "Система оценки кандидатов",
    submitting: "Отправка...",
    back: "Назад",
    refresh: "Обновить"
  },
  dashboard: {
    recent_activity: "Недавняя активность",
    latest_activity: "Последние действия кандидатов",
    no_activity: "Нет недавней активности",
    statistics: "Статистика",
    total_tests: "Всего тестов",
    active_tests: "Активные тесты",
    total_candidates: "Всего кандидатов",
    pending_sessions: "Ожидающие сессии",
    completed_sessions: "Завершенные сессии",
    view_all: "Посмотреть все",
    quick_actions: "Быстрые действия",
    create_test_description: "Добавить вопросы, установить ограничения по времени и многое другое",
    view_candidates: "Просмотр кандидатов",
    view_candidates_description: "Просмотр прогресса и результатов кандидатов",
    configure_settings: "Настройки",
    configure_settings_description: "Управление настройками учетной записи"
  },
  candidates: {
    name: "Имя",
    email: "Email",
    position: "Должность",
    status: "Статус",
    no_position: "Нет должности",
    assign_test: "Назначить тест",
    no_tests: "Нет тестов",
    test_results: "Результаты теста",
    test_name: "Название теста",
    score: "Счёт",
    pass: "Сдан",
    fail: "Не сдан",
    in_progress: "В процессе",
    pending: "Ожидает",
    completed: "Завершено",
    passed: "Сдано",
    failed: "Не сдано",
    mixed_results: "Смешанные результаты",
    add_candidate: "Добавить кандидата",
    edit_candidate: "Редактировать кандидата",
    not_found: "Кандидат не найден",
    not_found_message: "Кандидат, которого вы ищете, не существует или был удален",
    back_to_candidates: "Вернуться к списку кандидатов",
    profile_and_history: "Профиль кандидата и история тестов",
    information: "Информация о кандидате",
    not_specified: "Не указано",
    registered: "Зарегистрирован",
    test_history: "История тестов",
    view_all_tests: "Все тесты, назначенные этому кандидату",
    assign_new_test: "Назначить новый тест",
    active_tests: "Активные тесты",
    completed_tests: "Завершенные тесты",
    all_tests: "Все тесты",
    no_active_tests: "Нет активных тестов для этого кандидата",
    no_completed_tests: "Нет завершенных тестов для этого кандидата",
    no_tests_assigned: "Этому кандидату еще не назначены тесты",
    test: "Тест",
    uncategorized: "Без категории",
    expires: "Истекает",
    started: "Начат",
    completed_at: "Завершен",
    copy_link: "Скопировать ссылку",
    points: "баллов",
    passing: "Проходной балл"
  },
  tests: {
    test_name: "Название теста",
    category: "Категория",
    questions: "Вопросы",
    status: "Статус",
    active: "Активен",
    archived: "Архивирован",
    create_test: "Создать тест",
    edit_test: "Редактировать тест",
    description: "Описание",
    time_limit: "Ограничение по времени",
    minutes: "минут",
    passing_score: "Проходной балл",
    assign_to_candidate: "Назначить кандидату",
    expires_at: "Истекает",
    question_count: "Количество вопросов",
    manage_questions: "Управление вопросами",
    add_question: "Добавить вопрос",
    edit_question: "Редактировать вопрос",
    no_questions: "Нет вопросов. Добавьте вопросы для этого теста.",
    search_placeholder: "Поиск тестов по названию или категории",
    test_details: "Детали теста",
    edit_description: "Редактировать детали теста и управлять вопросами",
    active_status: "Активен",
    make_available: "Сделать этот тест доступным для кандидатов",
    enter_test_name: "Введите название теста",
    enter_test_description: "Введите описание теста",
    select_category: "Выберите категорию",
    leave_empty_for_no_limit: "Оставьте пустым для отсутствия ограничения",
    test_created_successfully: "Тест успешно создан",
    test_updated_successfully: "Тест успешно обновлен",
    failed_to_create: "Не удалось создать тест",
    failed_to_update: "Не удалось обновить тест",
    status_updated: "Статус теста успешно обновлен",
    failed_to_update_status: "Не удалось обновить статус теста"
  },
  question_types: {
    single_choice: "Один вариант",
    multiple_choice: "Несколько вариантов",
    true_false: "Верно/Неверно",
    short_answer: "Короткий ответ",
    essay: "Эссе"
  },
  settings: {
    profile: "Профиль",
    security: "Безопасность",
    notifications: "Уведомления",
    language: "Язык",
    select_language: "Выберите язык",
    dark_mode: "Темный режим",
    save_changes: "Сохранить изменения"
  },
  languages: {
    ru: "Русский",
    en: "Английский"
  }
};

const enTranslation = {
  common: {
    dashboard: "Dashboard",
    tests: "Tests",
    candidates: "Candidates",
    settings: "Settings",
    admin: "Admin",
    hr_manager: "HR Manager",
    loading: "Loading...",
    filter: "Filter",
    search: "Search...",
    no_data: "No data",
    no_filter_results: "No results match your filter criteria",
    actions: "Actions",
    edit: "Edit",
    delete: "Delete",
    save: "Save",
    cancel: "Cancel",
    success: "Success",
    error: "Error",
    app_name: "Candidate Assessment System",
    submitting: "Submitting...",
    back: "Back",
    refresh: "Refresh"
  },
  dashboard: {
    recent_activity: "Recent Activity",
    latest_activity: "Latest candidate test activity",
    no_activity: "No recent activity",
    statistics: "Statistics",
    total_tests: "Total Tests",
    active_tests: "Active Tests",
    total_candidates: "Total Candidates",
    pending_sessions: "Pending Sessions",
    completed_sessions: "Completed Sessions",
    view_all: "View All",
    quick_actions: "Quick Actions",
    create_test_description: "Add questions, set time limits, and more",
    view_candidates: "View candidates",
    view_candidates_description: "Review candidate progress and results",
    configure_settings: "Configure settings",
    configure_settings_description: "Manage your account preferences"
  },
  candidates: {
    name: "Name",
    email: "Email",
    position: "Position",
    status: "Status",
    no_position: "No position",
    assign_test: "Assign Test",
    no_tests: "No tests",
    test_results: "Test Results",
    test_name: "Test Name",
    score: "Score",
    pass: "Pass",
    fail: "Fail",
    in_progress: "In Progress",
    pending: "Pending",
    completed: "Completed",
    passed: "Passed",
    failed: "Failed",
    mixed_results: "Mixed Results",
    add_candidate: "Add Candidate",
    edit_candidate: "Edit Candidate",
    not_found: "Candidate Not Found",
    not_found_message: "The candidate you're looking for doesn't exist or has been removed",
    back_to_candidates: "Back to Candidates",
    profile_and_history: "Candidate profile and test history",
    information: "Candidate Information",
    not_specified: "Not specified",
    registered: "Registered",
    test_history: "Test History",
    view_all_tests: "View all tests assigned to this candidate",
    assign_new_test: "Assign New Test",
    active_tests: "Active Tests",
    completed_tests: "Completed Tests",
    all_tests: "All Tests",
    no_active_tests: "No active tests found for this candidate",
    no_completed_tests: "No completed tests found for this candidate",
    no_tests_assigned: "No tests assigned to this candidate yet",
    test: "Test",
    uncategorized: "Uncategorized",
    expires: "Expires",
    started: "Started",
    completed_at: "Completed",
    copy_link: "Copy Link",
    points: "pts",
    passing: "Passing"
  },
  tests: {
    test_name: "Test Name",
    category: "Category",
    questions: "Questions",
    status: "Status",
    active: "Active",
    archived: "Archived",
    create_test: "Create Test",
    edit_test: "Edit Test",
    description: "Description",
    time_limit: "Time Limit",
    minutes: "minutes",
    passing_score: "Passing Score",
    assign_to_candidate: "Assign to Candidate",
    expires_at: "Expires At",
    question_count: "Question Count",
    manage_questions: "Manage Questions",
    add_question: "Add Question",
    edit_question: "Edit Question",
    no_questions: "No questions. Add questions for this test.",
    search_placeholder: "Search tests by name or category",
    test_details: "Test Details",
    edit_description: "Edit test details and manage questions",
    active_status: "Active Status",
    make_available: "Make this test available for candidates",
    enter_test_name: "Enter test name",
    enter_test_description: "Enter test description",
    select_category: "Select a category",
    leave_empty_for_no_limit: "Leave empty for no limit",
    test_created_successfully: "Test created successfully",
    test_updated_successfully: "Test updated successfully",
    failed_to_create: "Failed to create test",
    failed_to_update: "Failed to update test",
    status_updated: "Test status updated successfully",
    failed_to_update_status: "Failed to update test status"
  },
  question_types: {
    single_choice: "Single Choice",
    multiple_choice: "Multiple Choice",
    true_false: "True/False",
    short_answer: "Short Answer",
    essay: "Essay"
  },
  settings: {
    profile: "Profile",
    security: "Security",
    notifications: "Notifications",
    language: "Language",
    select_language: "Select Language",
    dark_mode: "Dark Mode",
    save_changes: "Save Changes"
  },
  languages: {
    ru: "Russian",
    en: "English"
  }
};

// Языковые ресурсы
const resources = {
  ru: {
    translation: ruTranslation
  },
  en: {
    translation: enTranslation
  }
};

// Инициализация i18next
i18n
  .use(initReactI18next)
  .init({
    resources,
    lng: "ru", // Язык по умолчанию
    fallbackLng: "ru", // Запасной язык
    interpolation: {
      escapeValue: false // Разрешаем React сам обрабатывать эскейпинг
    }
  });

export default i18n;